...
SNUK_INLINE bool snuk_is_alpha_numeric(char c) {
    return snuk_is_alpha(c) || (c >= '0' && c <= '9');
}

SNUK_INLINE bool snuk_is_digit(char c) {
    return (c >= '0' && c <= '9');
}

SNUK_INLINE bool snuk_is_binary_digit(char c) {
    return c == '0' || c == '1';
}

SNUK_INLINE bool snuk_is_octal_digit(char c) {
    return (c >= '0' && c <= '7');
}

SNUK_INLINE bool snuk_is_hex_digit(char c) {
    if (c >= '0' && c <= '9') return true;
    c |= (1 << 5);
    return c >= 'a' && c <= 'f';
}

SNUK_INLINE char snuk_lower_case(char c) {
    if (snuk_is_alpha(c)) return c | (1 << 5);
    return c;
}

SNUK_INLINE char snuk_upper_case(char c) {
    if (snuk_is_alpha(c)) return c & ~(1 << 5);
    return c;
}
...
SNUK_INLINE bool snuk_string_equal_ignore_case(const char *a, const char *b) {
    if (!a || !b) return false;
