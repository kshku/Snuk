 ...
 */
u64 sStringLengthC8(const c8 *str) {
    u64 len = 0;
    while (str[len]) ++len;
    return len;
}
 ...
 */
c8 *sStringConcatC8(const c8 *str1, const c8 *str2, u64 l1, u64 l2,
                    u64 *length) {
    if (!l1) l1 = sStringLengthC8(str1);
    if (!l2) l2 = sStringLengthC8(str2);

    c8 *str = (c8 *)sCalloc(l1 + l2 + 1, sizeof(c8));
    sMemCopy((void *)str, (void *)str1, l1 * sizeof(c8));
    sMemCopy((void *)(str + l1), (void *)str2, l2 * sizeof(c8));
 ...
 */
b8 sStringEqualC8(const c8 *str1, const c8 *str2, u64 len) {
 ...
 */
c8 *sStringCopyC8(const c8 *str, u64 len) {
    if (!len) len = sStringLengthC8(str);

    c8 *s = (c8 *)sCalloc((len + 1), sizeof(c8));
    sMemCopy((void *)s, (void *)str, len * sizeof(c8));
