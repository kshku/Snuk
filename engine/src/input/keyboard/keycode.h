...
#define DEFINE_KEYCODE(key, value) CONCAT(EXPAND(KEYCODE_PREFIX), key) = value
 ...
 */
#define DEFINE_KEYCODE_FROM_SCANCODE(key) \
    DEFINE_KEYCODE(key, (CONCAT(EXPAND(SCANCODE_PREFIX), key)) | SCANCODE_MASK)
...
#define DEFINE_KEYMOD(mod, value) CONCAT(EXPAND(KEYMOD_PREFIX), mod) = value
...
#define KEYMOD(key) CONCAT(EXPAND(KEYMOD_PREFIX), key)
...
#undef KEYMOD
    ...
    // #define SDL_KMOD_NUM    0x1000u /**< the Num Lock key (may be located on
    // an extended keypad) is down. */
