...
typedef struct SnukTest {
    ...
    #define SNUK_TEST_SECTION \
        __attribute__((used, section("__DATA,snuk_tests")))
...
#define ADD_TEST(fn)                                        \
    ...
    static SNUK_TEST_SECTION SnukTest *snuk_test_ptr_##fn = \
        &snuk_test_struct_##fn;                             \
    static bool fn(void)
...
#define ASSERT(x)                                                             \
    do {                                                                      \
        if (!(x)) {                                                           \
            log_error(                                                        \
                "Assertion failed: %s (%s:%d) in %s", #x, __FILE__, __LINE__, \
                __func__);                                                    \
            TEST_FAILED;                                                      \
        }                                                                     \
    } while (0)
...
#define ASSERT_EQ(a, b)                                                       \
    do {                                                                      \
        if ((a) != (b)) {                                                     \
            log_error(                                                        \
                "Assertion failed: %s == %s (%s:%d) in %s", #a, #b, __FILE__, \
                __LINE__, __func__);                                          \
            TEST_FAILED;                                                      \
        }                                                                     \
    } while (0)
...
#define ASSERT_NULL(x)                                                        \
    do {                                                                      \
        if ((x) != NULL) {                                                    \
            log_error(                                                        \
                "Assertion failed: expected %s to be NULL (%s:%d) in %s", #x, \
...
#define ASSERT_NOT_NULL(x)                                                    \
    do {                                                                      \
        if ((x) == NULL) {                                                    \
            log_error(                                                        \
                "Assertion failed: expected %s to be non-NULL (%s:%d) in %s", \
...
#define ASSERT_PTR_EQ(a, b)                                                  \
    do {                                                                     \
        if ((void *)(a) != (void *)(b)) {                                    \
            log_error(                                                       \
                "Assertion failed: pointers %s == %s (%s:%d) in %s", #a, #b, \
                __FILE__, __LINE__, __func__);                               \
            TEST_FAILED;                                                     \
        }                                                                    \
    } while (0)
    ...
    do {                                                           \
        ...
        if (snuk_char_a != snuk_char_b) {                          \
            log_error(                                             \
                ...
                "  left : '%c' (%d)\n"                             \
                "  right: '%c' (%d)",                              \
                #a, #b, __FILE__, __LINE__, __func__, snuk_char_a, \
                snuk_char_a, snuk_char_b, snuk_char_b);            \
            TEST_FAILED;                                           \
        }                                                          \
    } while (0)
