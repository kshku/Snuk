...
SnukValue builtin_int_get_member(SnukValue value, SnukStringView field) {
    SNUK_ASSERT(value.type == SNUK_VALUE_INT, "something went wrong");
    for (uint64_t i = 0; i < SNUK_ARRAY_LENGTH(int_members); ++i)
        if (snuk_string_view_equal_cstr(field, int_members[i].field))
            return int_members[i].build_field(value);
...
static SnukValue build_to_int(SnukValue value) {
    ...
    Parameters params[] = {
        {.name = self, .type = &int_type, value}, // self should be last to add
    };
...
static SnukValue to_float(SnukInterpreter *intpret) {
    ...
    return (SnukValue){
        ...
        .float_value = (double)self_env->value.int_value,
    };
}
...
static SnukValue to_bool(SnukInterpreter *intpret) {
    ...
    return (SnukValue){
        ...
        .bool_value = (bool)self_env->value.int_value,
    };
}
...
static SnukValue to_str(SnukInterpreter *intpret) {
    ...
    uint64_t len = 0;
    len = snprintf(buf, 25, "\"%" PRId64 "\"", self_env->value.int_value);
    buf[len] = 0;
    return (SnukValue){
